...
import { pageObjectTemplate } from "./pageObjectTemplatePerfTest";
...
export const getPage = async (locators: Array<Locator>, title: string, url: string) => {
  //const res = await getPageAttributes();
  //const { title1, url } = res[0].result;
  const pageObject = pageObjectTemplate(locators, title, url);
  return pageObject;
};

export const generatePageObjectPerfTest = async (elements: Array<Locator>, title: string, url: string) => {
  const page = await getPage(elements, title, url);
